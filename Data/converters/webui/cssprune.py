...
# ── защита от преждевременного закрытия `<style>` ──────────────────────────────
# `</style` (регистронезависимо, за которой идёт граница тега: пробел/`/`/`>`/конец
# текста) обязана быть экранирована ПЕРЕД тем, как css_text уедет внутрь <style> —
# см. guard_style_close ниже.
_STYLE_CLOSE_RE = re.compile(r"<(/style(?=[\s/>]|\Z))", re.I)


def guard_style_close(css_text):
    """Не дать буквальной `</style` внутри CSS преждевременно закрыть внешний `<style>`.

    HTML-токенайзер в состоянии RAWTEXT (это ровно то состояние, в котором браузер
    читает содержимое `<style>`) ищет подстроку `</style` ЛИТЕРАЛЬНО, посимвольно —
    ему не важно, что по CSS-синтаксису эти символы лежат внутри строки
    (`content: "...</style>..."`) или комментария. Как только он видит `</style` с
    границей тега следом, тег закрывается — независимо от того, была ли это часть
    CSS-значения.

    Источник риска — не гипотетический: `build_html` собирает `<style>` из колонки
    `css`, взятой с ЖИВЫХ сайтов (документация, style-guide'ы, туториалы), а не
    из машинно-сгенерированного CSS (в отличие от websight/webcode2m). У таких
    страниц `content:` в `::before`/`::after` нередко показывает пример разметки
    («вот как выглядит код») — ровно то место, где может встретиться `</style>`.

    Цена срабатывания без защиты — не просто «CSS обрезался». Хвост стайлшита
    вытекает в тело страницы как ВИДИМЫЙ мусорный текст (сломанная вёрстка попадает
    в тренировочную пару: и в скриншот, и в html), а всё, что шло в исходном HTML
    ПОСЛЕ `<style>`, парсится заново с нуля — в том числе `<script>`, который
    `sanitize_html` уже вырезал ДО этого момента (задняя дверь мимо санитайза).
    Хуже: `accept_page` (шаг 6) сравнивает рендер ДО и ПОСЛЕ шейкинга, а оба идут
    через один и тот же `build_html` — если хвост, содержащий `</style>`, уцелел
    в обоих (что обычно и происходит: правило, куда он вложен, либо выживает в
    обеих версиях, либо не выживает ни в одной), поломка одинакова с обеих сторон,
    `pixel_sim` = 1.0, и приёмка проходит молча.

    Фикс — вставить невидимый ZWSP (`\u200b`) между `<` и `/`. HTML-токенайзер
    перестаёт видеть смежную пару символов `</` и не входит в состояние закрытия
    тега. CSS ZWSP не замечает вовсе — что в строке, что в комментарии, что в
    `url(...)`: это не спецсимвол синтаксиса ни в одном контексте CSS.
    """
    return _STYLE_CLOSE_RE.sub("<\u200b\\1", css_text or "")
...
def _split_decls(body):
    """Разбить тело правила на объявления.

    Комментарии выбрасываются, а не переносятся в объявление. Иначе закомментированная
    строка считалась бы объявлением (`css_decls` — признак сложности и барьер «страница
    без стилей», так что лишний счёт двигает отбор), а комментарий с `;` внутри рвал бы
    настоящее объявление надвое при пересборке в treeshake_css.
    """
