...
PLACEHOLDER_RGB = (209, 213, 219)      # #d1d5db — канонический серый блок вместо <img>


def ink_features(png_path, max_side=300, tol=6, white=245):
    """Чем ЗАПОЛНЕНА страница: доля серых плейсхолдеров, белого поля и «чернил».

    `ink` = площадь, которая не белая и не плейсхолдер, то есть реально нарисованный
    контент. Признак нужен потому, что структурная сложность про это не знает НИЧЕГО:
    замер на отобранном WebCode2M дал у топ-200 по `complexity` медиану чернил 15.6%,
    у нижних-200 — 15.1%. Разницы нет, то есть страница может быть на 500 узлов и при
    этом на две трети пустой.

    Медианы ПОЛНЫМ ПРОХОДОМ: синтетика 56.9% (n=124), WebUI 13.0% (n=1 582),
    WebCode2M-complex 13.2% (n=4 714, при 16.3% серых плейсхолдеров).

    ⚠ Распределение БИМОДАЛЬНО (у WebUI 52.8% страниц ниже 15% чернил и 18.9% выше
    90%), поэтому медиану по подвыборке брать нельзя: на выборках в 120 страниц она
    давала 38.2 / 21.0 / 32.0% при истинных 19.6%. Считать только полным проходом и
    смотреть на доли по порогу. (Прежняя редакция этого docstring'а несла именно такие
    выборочные числа — «WebUI 44.9%» — и они неверны.)

    ⚠ Чем это НЕ является: порогом «интересности». Страницы с самыми высокими чернилами
    в WebCode2M — SEO-помойки с цветным фоном и календарные таблицы. Признак отделяет
    пустое от залитого, а не осмысленное от мусорного; для второго нужен семантический
    судья, а не пиксели.
    """
    try:
        import numpy as np
        ...
        im.thumbnail((max_side, 10000))      # доли площади от уменьшения не плывут
        a = np.asarray(im).reshape(-1, 3)
        total = max(1, len(a))
        ph = float(np.all(np.abs(a.astype(int) - np.array(PLACEHOLDER_RGB)) <= tol,
                          axis=1).sum() / total)
        wh = float(np.all(a >= white, axis=1).sum() / total)
        return {"placeholder_share": round(ph, 4), "white_share": round(wh, 4),
                "ink": round(1.0 - wh - ph, 4)}
    except Exception:
        return {}
