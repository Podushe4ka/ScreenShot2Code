...
# в Evaluation/render.py подменяет запросы по подстроке URL на локальный файл, а всё
# остальное внешнее делает route.abort(). Имена файлов здесь обязаны совпадать со
# значениями _VENDOR_MAP (Evaluation/render.py:317) — иначе перехват молча не сработает
# и страница отрендерится пустой.
#
# Использование:
