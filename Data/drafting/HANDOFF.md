...
| визуальные токены (оценка) | ~960 | ~1 672 | ~1 672 |
| **всего** | ~3 409 | **~5 305** | ~6 485 |

→ **рекомендуемый `max_length` = 6144** (влезает в SFT-окно 8192 с запасом).
Пересчёт: `--token-report` (тянет transformers/torch).
