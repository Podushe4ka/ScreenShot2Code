...
| **3а. Код после конвертера** median | — | — | **2 664** ⁵ᵃ | — |
...
> читаемости — [`pixel_budget.py`](tools/pixel_budget.py); числа ниже — для старого бюджета
...
стайлшитом сайта.** После конвертера (14 авг) распределение другое: медиана **2 664**
токена на 1 582 принятых страницах, и барьер бюджета при окне 16384 снимает 55 страниц.
Отсечка по длине из ограничения №1 выбыла.

⚠ **Не замерено: p99 и max кода после конвертации.** В журнале есть только медиана.
Досчитать — по манифесту staging'а тем же инструментом, что и остальную строку.
Пока этого нет, планировать `max_length` под WebUI по числам сырого источника **нельзя**:
они завышают хвост на два порядка.
