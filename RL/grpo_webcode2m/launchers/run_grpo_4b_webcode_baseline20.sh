...
VAL_DATA="$STORAGE_ROOT/data/verl_webcode2m_20/validation.parquet"
...
TRAINER=(
    ...
    ++ray_kwargs.ray_init._temp_dir=$STORAGE_ROOT/r4bb20
    trainer.experiment_name=qwen35_4b_webcode_baseline20
    trainer.default_local_dir="$STORAGE_ROOT/checkpoints/grpo_4b_webcode_baseline20"
    trainer.rollout_data_dir="$PROJECT/outputs/grpo_4b_webcode_baseline20/rollouts"
    trainer.validation_data_dir="$PROJECT/outputs/grpo_4b_webcode_baseline20/validation"
)

mkdir -p \
    "$PROJECT/logs/grpo_4b_webcode_baseline20" \
    "$PROJECT/checkpoints/grpo_4b_webcode_baseline20"
...
python -m verl.trainer.main_ppo \
    ...
    "${ROLLOUT[@]}" \
    "${TRAINER[@]}" \
    2>&1 | tee "$PROJECT/logs/grpo_4b_webcode_baseline20/run.log"