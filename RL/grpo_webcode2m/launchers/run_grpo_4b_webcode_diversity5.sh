...
VAL_DATA="$STORAGE_ROOT/data/verl_webcode2m_20/validation5.parquet"
...
TRAINER=(
    ...
    ++ray_kwargs.ray_init._temp_dir=$STORAGE_ROOT/r4bd5
    trainer.experiment_name=qwen35_4b_webcode_diversity5
    trainer.default_local_dir="$STORAGE_ROOT/checkpoints/grpo_4b_webcode_diversity5"
    trainer.rollout_data_dir="$PROJECT/outputs/grpo_4b_webcode_diversity5/rollouts"
    trainer.validation_data_dir="$PROJECT/outputs/grpo_4b_webcode_diversity5/validation"
)

mkdir -p \
    "$PROJECT/logs/grpo_4b_webcode_diversity5" \
    "$PROJECT/checkpoints/grpo_4b_webcode_diversity5"
...
python -m verl.trainer.main_ppo \
    ...
    "${ROLLOUT[@]}" \
    "${TRAINER[@]}" \
    2>&1 | tee "$PROJECT/logs/grpo_4b_webcode_diversity5/run.log"