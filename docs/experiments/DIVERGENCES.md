...
| K6 | `run_info.yaml` больше не пишется | `SFT/train/run_info.py` умеет сохранять эффективные аргументы, бюджет токенов, число отбракованных и git-коммит рядом с весами | `SFT/train/train_sft.py` собирает `meta`, но только печатает его и шлёт в ClearML; `save_run_info` не вызывается ниоткуда | [Июльская линия](2026-07-30-july-line-webcode2m.md) прямо говорит «конфиги восстанавливаются из `run_info.yaml` рядом с весами» — для новых прогонов это уже неверно, и восстановить конфиг по весам нельзя | модуль возвращён в ветку с архивной; **подключение к обучению — решение владельца `SFT/`**, это правка поведения |
...
| E2 | прогон судьи ни с чем у нас не смыкается | `judge_one_gpu/bench_results/summary.json`: `final_score 0.8955`, `judge_winrate` чекпоинт 0.456 / база 0.544, 2000 сэмплов | наши таблицы — `metrics_only`, Design2Code-484, наши чекпоинты | Различий три сразу: чекпоинт из чужого каталога (`ScreenShot2Code` с большой `S`, рецепт неизвестен), набор 2000 вместо 484, рендер без четырёх фиксов. Направление совпало с E12, но это совпадение, а не подтверждение | навести судью на наши чекпоинты — тогда сравнение станет осмысленным |
