...
8 ТБ, общий между **a100-2** и **a100-3**. Рабочий каталог —
`/mnt/storage-1/Screenshot2Code` (маленькая `s`).

⚠ Рядом лежит `/mnt/storage-1/ScreenShot2Code` — с большой `S`. **Это тоже наш
каталог**, расхождение в регистре тянется с разных машин, а не от другого
человека. Раньше здесь было написано, что большая `S` — чужой каталог; это
неверно, и по этой ошибке уже был сделан неправильный вывод о принадлежности
чекпоинта. Оба пути наши, но это **разные каталоги**, и путь надо копировать
как есть, а не набирать по памяти.
