...
Источник: https://github.com/NoviScl/Design2Code (metrics/visual_score.py,
metrics/ocr_free_utils.py, metrics/screenshot_single.py, data_utils/dedup_post_gen.py),
код скопирован в один самостоятельный файл — git clone оригинала не нужен.
Рендер (screenshot_single.py) инлайнен как обычная функция и делегирует в
render.py (общий браузер на процесс, см. render.py) вместо subprocess на
каждый скриншот.

Единственное сознательное отличие от оригинала: final_score — среднее
геометрическое пяти метрик, а не среднее арифметическое (оригинальная
формула сохранена рядом как final_score_arithmetic, для сверки).
...
# =============================================================================
# take_screenshot/render_many из render.py: один общий процесс браузера на
# весь прогон (см. render._get_browser) — открытие/закрытие вкладки, а не
# запуск нового Chromium на каждый скриншот. get_blocks_ocr_free ниже зовёт
# рендер дважды на сэмпл (p_png, p_png_1) через один render_many — конкурентно
# на общем браузере.
...
def get_ref_blocks_cached(ref_html_path: str, cache_path: str = None):
    ...
    считаем их один раз и кэшируем на диск рядом с ref.html; повторные вызовы
    для того же ref.html читают JSON без единого обращения к браузеру.
...
def visual_eval_v3_multi(input_list, debug=False, original_blocks=None, original_img=None):
    ...
    # На сэмпл может быть до 6 рендеров (pred.png + pred_p.png + pred_p_1.png,
    # и то же самое для ref, если кэш не передан) — ни один не зависит от
    # результата другого (все получаются из HTML, никто не читает PNG
    # другого рендера), так что все уходят в один render_many и рендерятся
    # конкурентно на общем браузере, а не последовательно один за другим.
    # Именно эта конкурентность порождает нагрузку на браузер, из-за которой
    # этап метрик чекпоинта чувствительнее к таймауту рендера, чем рендер
    # baseline (там на сэмпл всего один скриншот) — см. --render-workers и
    # D2C_RENDER_TIMEOUT_MS в run_benchmark_batched.py/render.py.
    all_jobs = []
