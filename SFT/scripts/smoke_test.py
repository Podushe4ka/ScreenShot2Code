...
# Картинка обязана УПИРАТЬСЯ в MAX_PIXELS, иначе check_visual_budget проверяет не то.
# Было 1280x1280 = 1.638 Мп при потолке 2.097 Мп: ужатия не происходило, картинка давала
# 1600 визуальных токенов при пороге 0.8*2048 = 1638.4, и проверка падала всегда — на
# любых промптах и любых данных, потому что число визуальных токенов от текста не зависит.
# 1280x2048 = 2.62 Мп — выше потолка, и это ровно тот предельный размер скриншота, который
# принимает конвейер синтетики (Data/converters/synth/build.py: MAX_HEIGHT).
LARGE_IMAGE = (1280, 2048)
