...
args+=(
  ...
  -e CUDA_MODULE_LOADING="${CUDA_MODULE_LOADING:-EAGER}"
...
for var in HF_TOKEN CLEARML_API_ACCESS_KEY CLEARML_API_SECRET_KEY \
           ...
           RESULT_DIR OUT DATASET CONFIG NPROC ONLY STEPS SKIP_EXISTING \
           EXTRA_ARGS RUN_ENV TRACE_PATH MEM_SNAPSHOT \
           CLEARML_TAGS SFT_MAX_PIXELS SFT_MIN_PIXELS; do
  if [[ -n "${!var:-}" ]]; then
    args+=(-e "$var=${!var}")
  fi
done
