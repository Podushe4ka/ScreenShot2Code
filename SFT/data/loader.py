...
Сплиты приходят из датасета (`train` / `test`), а не режутся здесь: имена
стандартные, поэтому один и тот же код работает и с нашими данными, и с любым
датасетом с Hub.
"""
...
def load_sft_dataset(
    ...
    `required=False` — вернуть None вместо ошибки, если сплита нет (нужно для
    eval: обучение без валидации допустимо).
    """
